...
name: hv-analysis
description: |
  纵横双轴分析法（Horizontal-Vertical Analysis）深度研究Skill。融合了语言学的历时-共时分析、社会科学的纵向-横截面研究设计、商学院案例研究法与竞争战略分析的核心思想。
  ...
  不要用于简单的名词解释（用户只是问"XX是什么"）、不要用于纯营销写作或品牌推广、不要用于纯标题摘要生成。
---

# 纵横双轴分析法深度研究

> **方法论说明**
> 纵横双轴分析法融合了语言学中的历时-共时分析（Saussure）、社会科学中的纵向-横截面研究设计、商学院案例研究法、以及竞争战略分析的核心思想，形成了一套适用于产品/公司/概念/人物的通用研究框架。核心原则：纵向追时间深度，横向追同期广度，最终交汇出独立判断。

你正在执行一次纵横双轴分析。最终产出一份**排版精美的PDF研究报告**。
...
这不是一份冷冰冰的咨询报告，而是一篇让人能从头读到尾的深度研究。写作风格需要在「研究报告的严谨」和「深度报告的可读性」之间找到平衡点。

### 深度报告的写作特征

以下风格元素是深度研究报告的核心特征，应该贯穿整篇报告：
...
**敢下判断**：鼓励给出观点和洞察，但每个观点必须有事实支撑。先摆事实，再给判断。是推测的明确标注。表达判断时用「我觉得」「据我判断」这种承认主观性的姿态，而不是居高临下的定论。
...
以下原则适合深度研究报告，需要与公众号文章风格区别对待：

- **保留明确的结构**：公众号文章的特点是一口气顺下来，最小化小标题。但10000-30000字的研究报告如果没有清晰的结构和导航，读者会迷路。报告需要完整的章节结构（H1、H2、H3）。
- **标点符号的正常使用**：公众号常禁用冒号和破折号，但研究报告应该正常使用，因为报告是为了信息传达效率。但对「」的使用习惯可以保留。
- **不需要营销尾部**：移除公众号的三连/星标尾部等营销元素。报告的结尾是有思想深度的洞察，不是行动号召。
...
3. **运行转换脚本**：
   ```bash
   python [skill目录]/scripts/md_to_pdf.py input.md output.pdf --title "研究对象名称" [--author "研究者名称"]
   ```
...
- **封面页**：自动生成，包含标题（28pt深蓝色）、副标题「纵横双轴分析法深度研究报告」、作者信息、装饰分隔线
